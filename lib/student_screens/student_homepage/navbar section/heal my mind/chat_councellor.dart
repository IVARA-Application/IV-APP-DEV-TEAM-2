...
import 'package:new_ivara_app/student_screens/student_homepage/navbar%20section/heal%20my%20mind/healMyMindsFunctions/chatCouncellorMethods.dart';
...
class ChatToCounsellorPage extends StatefulWidget {
  ...
  int _class = 6;
  ChatToCounsellorPage({Key key}) : super(key: key);

  @override
  _ChatToCounsellorPageState createState() => _ChatToCounsellorPageState();
}
...
class _ChatToCounsellorPageState extends State<ChatToCounsellorPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      onWillPop: () {
        print("ON will pop scoped called");
        chatRoomReference.get().then((snapshot) {
          snapshot.docs.forEach((doc) {
            if (doc.exists) {
              doc.reference.delete();
            }
                                        ...
                                        children: [
                                          ...
                                          IconButton(
                                            icon: Icon(Icons.attach_file,
                                                color: Color(0xFF697AE4)),
                                            onPressed: () {
                                              ChatToCouncellorMethods.sendFiles(
                                                  chatRoomReference,
                                                  Get.find<AuthController>()
                                                      .user
                                                      .value
